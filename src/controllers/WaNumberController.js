...
class WaNumberController {
    ...
    static async getAll(req,res) {
        try {
            // service logic
            const dt = await WaNumberService.getAll(req.login_info);
    ...
    static async downloadTemplate(req,res) {
        try {
            // template path
            const filePath  = path.join(__dirname, '..', 'uploads', 'import_number_template.xlsx');
    ...
    static async importExcel(req, res) {
        try {
            ...
            formatResponse(res, dtCreateBulk.code, dtCreateBulk.message, dtCreateBulk.data);
        } catch (error) {
            formatResponse(res, 500, error.message);
        }
    }
    ...
    static async delete(req,res) {
        try {
            // service logic            
            const dtDelete = await WaNumberService.delete(req.params.wa_number_id, req.login_info);
